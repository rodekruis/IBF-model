server <- function(input, output) {
  ...
  selected_pcode <- reactiveVal("ET0101")
  ...
  RAIN_PCODE2 <- reactive({
    req(input$Season_Obs_Rain)
    rainfall_sesonal_df %>%
      filter(ADM2_PCODE == selected_pcode(),
  ...
  csi.bins <-c(0, 0.2, 0.3, 0.55, 1.0) 
  #pal <- colorBin( "plasma", bins=risk.bins, na.color = "#f0f0f0")

  output$skill_map <- renderLeaflet({
    pal <- colorBin("BuPu", domain = skill_table()$CSI, bins =csi.bins,na.color = "#f0f0f0")
  ...
  output$result_html <- renderUI({
    HTML(
      paste0(
        '<span style="font-size:14px"> based on past impact data, how many time would we have acted in Vain: </span> ', droughts_incorrect_val(), "<br />",
        '<span style="font-size:14px"> based on past impact data, how many time would we have activated EAP correctly: </span> ', protocol_triggered_val(), "<br />",
        '<span style="font-size:14px"> Detection Ratio: </span> ', detection_ratio_val(), "<br />",
        '<span style="font-size:14px"> False Alarm Ratio: </span> ', false_alarm_ratio_val(), "<br />"
      )
    )
  })
