header <- dashboardHeader(
  ...
  tags$li(div(
    ...
    img(#src = 'https://www.510.global/wp-content/uploads/2017/07/510-LOGO-WEBSITE-01.png',#'510logo.png', title = "logo", height = "44px")
        src = 'https://redcross.or.ke/images/Media_Center/krc_logo.jpeg',
...
ui_tab_main <- tabItem(
  ...
  fluidRow(
    column(
      ...
      leafletOutput("impact_map", height=300),
      ...
      #fluidRow (
        column(
          width = 12,
              # leafletOutput("skill_map", height=300),
              leafletOutput("skill_map", height=300)),
       # )
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      radioButtons("country", "Country:", c("Kenya" = 1)),
      #radioButtons("country", "Country:", c("Ethiopia" = 1, "Kenya" = 2, "Mozambique" = 3,"Lesotho" = 4,"Namibia" = 5)),
      selectInput("Level", "Select aggregation  Level(for now only Admin ):", c("Counties"=1, "Livelihood zones"=10),selected=1),#"LEVEL 3"),
      selectInput("Season_Obs_Rain", "Select rainfall season:", c("OND" = 1,"MAM"=2),selected=1),
